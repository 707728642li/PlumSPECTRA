...
def main() -> None:
    ...
    manifest = {
        "cycle": "V9 bounded loss-profile completion on primary cohort",
        "target": TARGET,
        "heldout_cultivars": DEVELOPMENT_HELDOUT.split(","),
        "cohort": "primary",
        "excluded_cultivars": ["6.11"],
        "baseline_profile": {"name": "balanced", "center_weight": 0.20, "rank_weight": 0.08},
        "variants": {
            "ABSOLUTE": {"center_weight": 0.0, "rank_weight": 0.0},
            "RANKING": {"center_weight": 0.35, "rank_weight": 0.15},
        },
    ...
    summarize = [
        ...
        "--run",
        f"BALANCED={project / 'results' / 'v7' / 'primary_suite' / 'primary_v5'}",
        "--run",
        f"ABSOLUTE={output_root / 'absolute'}",
        "--run",
        f"RANKING={output_root / 'ranking'}",
        "--output-dir",
